...
const headers = [
    ...
    { label: "Date created", key: "dateupdated" },
...
class Companies extends React.Component {
    ...
    async getCustomers(pagenum) {
        ...
        let apiResponse = await APIService.makeApiGetRequest("company/0/10");
    ...
    async getCustomerReport() {
        //call API
        const notification = this.notificationSystem.current;
        let apiResponse = await APIService.makeApiGetRequest("company");
    ...
    async approveCustomer() {
        ...
        if (!privilegeList.includes("approve_registered_customer")) {
            ...
            if (!this.state.isApproved && (this.state.remarks == null || this.state.remarks === '')) {
                ...
                let result = await APIService.makePostRequest("company/update_status", params);
                if (result.success) {
                    notification.addNotification({
    ...
    async onClickCustomerSelected(row) {
        this.setState({
            ...
            krafilename: row.krafilename,
            krafiletype: row.krafiletype,
            openUpdate: true,
            updated_hover: true,
        });
        ...
        // });
        let param = {}
        param["filename"] = row.krafilename
        param["data"] = row.kra_pin_certificate
        fileData.push(param)
    ...
    handleCustomerSearch(e) {
        ...
        let searchResult = this.state.customer.filter(s => {
            return s.status.includes(value) || s.email.includes(value) || s.companyname.includes(value);
        });
        this.setState({
    ...
    generatePDF() {
        ...
        const tableColumn = ["Name", "Email", "Contact", "Status", "UpdatedOn", "CreatedOn", "DateUpdated"];
        ...
        doc.text("Sonysugar Company Registration Requests", 14, 15);
        const date = Date().split(" ");
        // we use a date string to generate our filename.
        const dateStr = date[0] + date[1] + date[2] + date[3] + date[4];
        // we define the name of our PDF file.
        doc.save(`Company_Registration_Requests${dateStr}.pdf`);
    };
    async AddPage() {
        this.setState({ show_progress_status: true });
    ...
    render() {
        ...
        return (
            <Aux>
                ...
                <Card title='Company registration' isOption>
                    <br />
                    <br />
                    <CSVLink data={this.state.report} headers={headers} filename='Company_Registration_Requests.csv'>

                        <FaFileExcel size={50} color='green' title='Download company registration requests' />
                    </CSVLink>
                    <IconButton onClick={() =>
                        this.generatePDF()
                    }>
                        <FaFilePdf title='Download company registration requests' size={50} color='red' />
                    </IconButton>
                    <IconButton onClick={() =>
                        this.RemovePage()
                    } >
                    ...
                    <div className="input-group mb-3">
                        <input type="text" className="form-control" style={{ color: '#000000' }} placeholder="You can search by company name, email or status" onChange={e => this.handleCustomerSearch(e)} />
                    </div>
                    ...
                    <div className="card">
                        ...
                        <div className="card-body text-center">
                            ...
                            {this.state.showmore ? <Row>
                                ...
                                <label style={{ color: '#000000' }}><b>Company name</b></label>
                                <div className="input-group mb-3">
                                    <input type="text" className="form-control" style={{ color: '#000000' }} value={this.state.companyname} readOnly />
                                </div>

                                <label style={{ color: '#000000' }}><b>Kra pin</b></label>
                                <div className="input-group mb-3">
                                    <input type="text" className="form-control" style={{ color: '#000000' }} value={this.state.krapin} readOnly />
                                </div>
